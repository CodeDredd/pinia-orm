...
In Pinia ORM you have that options that each model saves meta data. This meta data contains folling properties:

- `createdAt`
- `updatedAt`

If you have enabled them either through global configuration `createORM({ model: { withMeta:true } })` for all models or by
the model property `static withMeta = true` for a single one, than meta data will be saved under the `_meta` prop.
