...
::alert{type=warning}
Since `v2.0.0` the decorators are standard [ECMAScript decorators](https://github.com/tc39/proposal-decorators). They require TypeScript >= 5.2 **without** the `experimentalDecorators` flag, and fields are declared with `!` instead of the `declare` keyword — see the [Typescript guide](/guide/getting-started/typescript).
::
...
export class User extends Model {
  ...
  @Attr(null) id!: number | null
  @Attr('John Doe') name!: string
  @Attr(false) active!: boolean
}
```
...
export class User extends Model {
  ...
  @Str('') name!: string
  @Str('', { notNullable: true }) address!: string | null
}
```
...
export class User extends Model {
  ...
  @Num(0, { notNullable: true }) roleId!: number | null
}
```
...
export class User extends Model {
  ...
  @Bool(false) active!: boolean
  @Bool(false, { notNullable: true }) visible!: boolean | null
}
```
...
class User extends Model {
  ...
  @Uid({ alphabet: '0123456789abcdef', size: 7 }) id!: string
}
```
...
class User extends Model {
  ...
  @Mutate((value: any) => value.toUpperCase()) @Attr('') name!: string
}
...
class User extends Model {
  ...
  @Cast(() => StringCast) @Attr('') name!: string
}
...
class User extends Model {
  ...
  @HasOne(() => Phone, 'userId') phone!: Phone | null
}
```
...
class Post extends Model {
  ...
  @BelongsTo(() => User, 'userId') user!: User | null
}
```
...
class Cluster extends Model {
  ...
  @Attr(null) nodeIds!: number[]
  @HasManyBy(() => Node, 'nodesId') nodes!: Node[]
}
```
...
class User extends Model {
  ...
  @MorphOne(() => Image, 'imageableId', 'imageableType') image!: Image | null
}
```
...
class Image extends Model {
  ...
  @Attr(null) imageableId!: number | null
  @Attr(null) imageableType!: string | null
  @MorphTo(() => [User, Post], 'imageableId', 'imageableType') imageable!: User | Post | null
}
```
...
class Video extends Model {
  ...
  @MorphToMany(() => Comment, () => Commantable, 'commentId', 'commentableId', 'commentableType') comments!: Comment[]
}
```
...
class User extends Model {
  ...
  @HasMany(() => Post, 'userId') @OnDelete('cascade') posts!: Post[]
}
```
