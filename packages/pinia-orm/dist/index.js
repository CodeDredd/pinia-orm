...
const Utils = require('./shared/pinia-orm.fc1ee287.cjs');
...
class Query {
  ...
  getFieldWhereForRelations(relation, callback = () => {
    ...
    const modelIdsByRelation = this.newQuery(this.model.$entity()).with(relation, callback).get(false).filter((model) => Utils.compareWithOperator(
      model[relation] !== void 0 ? Utils.isArray(model[relation]) ? model[relation].length : model[relation] === null ? 0 : 1 : Utils.throwError(["Relation", relation, "not found in model: ", model.$entity()]),
  ...
  hydrate(records, options) {
    return Utils.isArray(records) ? records.map((record) => this.hydrate(record), options) : this.checkAndGetSTI(records, { relations: false, ...options || {} });
  }
  compile(models) {
  ...
  match(relation, models, query) {
    ...
    models.forEach((parentModel) => {
      ...
      relatedModels.forEach((relatedModel) => {
        ...
        const relatedModelCopy = ___default.cloneDeep(relatedModel);
